...
// Constants for window dimensions and validation layers
const uint32_t WIDTH = 800;
const uint32_t HEIGHT = 600;
const int MAX_FRAMES_IN_FLIGHT = 2;

// Validation layers are essential for catching errors in Vulkan development.
...
class HelloTriangleApplication {
 ...
 private:
  ...
  // Initialize all the Vulkan components
  void initVulkan() {
    createInstance();
    setupDebugMessenger();
    createSurface();
  ...
  // Recreate the swap chain if the window is resized
  ...
  // Find the right memory type for a buffer
  ...
  // Helper function to copy one buffer to another
  ...
  // Find the queue families for a physical device
