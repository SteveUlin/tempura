...
auto main() -> int {
  ...
  "Multiplication term comparison: different bases sorted separately"_test =
      [] {
        ...
        // The key insight: all x powers will be adjacent, all y powers will be
        // adjacent
        ...
        constexpr auto cmp2 =
            compareMultiplicationTerms(pow(x, 2_c), pow(x, 3_c));
        static_assert(cmp2 == Ordering::Less);
      };
  ...
  "Canonical ordering: x^2 · x → x · x^2"_test = [] {
    ...
    auto result =
        MultiplicationRuleCategories::Ordering.apply(expr, default_context());
  ...
  "Canonical ordering: y · x when bases differ"_test = [] {
    ...
    auto result1 =
        MultiplicationRuleCategories::Ordering.apply(expr1, default_context());
    auto result2 =
        MultiplicationRuleCategories::Ordering.apply(expr2, default_context());
  ...
  "Associativity groups like bases: x · (x^2 · y)"_test = [] {
    ...
    auto result = MultiplicationRuleCategories::Associativity.apply(
        expr, default_context());
  ...
  "Power combining: x · x^2 → x^3"_test = [] {
    ...
    auto result = MultiplicationRuleCategories::PowerCombining.apply(
        expr, default_context());
  ...
  "Complex expression: 2 · x^2 · y · 3 · x · y^3 · x^2"_test = [] {
    ...
    // Expected grouping: constants together, x powers together, y powers
