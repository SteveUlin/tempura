...
public:
  ...
  explicit Vec512i64(int64_t i) : value_(_mm512_set1_epi64(i)) {}
  explicit Vec512i64(const int64_t* ptr) : value_(_mm512_loadu_si512(ptr)) {}
  explicit Vec512i64(const std::array<int64_t, 8>& arr)
  ...
  void store(int64_t* ptr) const {
    _mm512_storeu_si512(ptr, value_);
  }

  auto storeArray() const -> std::array<int64_t, 8> {
  ...
  auto operator[](std::size_t index) const -> int64_t {
    ...
    return reinterpret_cast<const int64_t*>(&value_)[index];
  }
