...
// Heap (std::vector) with optionally-static dims: storage and dim-staticness
// are independent axes, so a large matrix can still carry a
// compile-time-checked shape.
template <typename T, std::size_t Rows = std::dynamic_extent,
          std::size_t Cols = std::dynamic_extent>
using Matrix = MdArray<T, std::extents<std::size_t, Rows, Cols>,
                       std::layout_right, std::vector<T>>;

template <typename T, std::size_t Rows, std::size_t Cols>
using InlineMatrix = MdArray<T, std::extents<std::size_t, Rows, Cols>,
                             std::layout_right, std::array<T, Rows * Cols>>;

constexpr auto mergeExtent(std::size_t a, std::size_t b) -> std::size_t {
  return a != std::dynamic_extent ? a : b;
}

// A dynamic (unknown) dim defers the check to runtime, so a static_assert on
// this fails only when both dims are known and unequal.
...
using SumExtents =
    std::extents<std::size_t,
                 mergeExtent(ExtA::static_extent(0), ExtB::static_extent(0)),
                 mergeExtent(ExtA::static_extent(1), ExtB::static_extent(1))>;
template <typename ExtA, typename ExtB>
using ProductExtents =
    std::extents<std::size_t, ExtA::static_extent(0), ExtB::static_extent(1)>;

// Value forms always own heap storage: result size isn't bounded by operand
// size (an outer product of two small stack matrices can be megabytes), so
// stack is opt-in via the destination form.
...
template <typename Ta, typename Ea, typename La, typename Ca, typename Tb,
          typename Eb, typename Lb, typename Cb, typename Td, typename Ed,
          typename Ld, typename Cd>
  requires(Ea::rank() == 2 && Eb::rank() == 2 && Ed::rank() == 2)
constexpr auto add(const MdArray<Ta, Ea, La, Ca>& a,
                   const MdArray<Tb, Eb, Lb, Cb>& b,
                   MdArray<Td, Ed, Ld, Cd>& dst) -> MdArray<Td, Ed, Ld, Cd>& {
  static_assert(dimsCompatible(Ea::static_extent(0), Eb::static_extent(0)),
                "row counts differ");
  static_assert(dimsCompatible(Ea::static_extent(1), Eb::static_extent(1)),
                "column counts differ");
  static_assert(dimsCompatible(Ed::static_extent(0), Ea::static_extent(0)),
                "dst rows mismatch");
  static_assert(dimsCompatible(Ed::static_extent(1), Ea::static_extent(1)),
                "dst columns mismatch");
  ...
  for (std::size_t i = 0; i < rows; ++i)
    for (std::size_t j = 0; j < cols; ++j)
      dst[i, j] = static_cast<Td>(a[i, j] + b[i, j]);
  return dst;
}
...
template <typename Ta, typename Ea, typename La, typename Ca, typename Tb,
          ...
          typename Ld, typename Cd>
  requires(Ea::rank() == 2 && Eb::rank() == 2 && Ed::rank() == 2)
constexpr auto multiply(const MdArray<Ta, Ea, La, Ca>& a,
                        const MdArray<Tb, Eb, Lb, Cb>& b,
                        MdArray<Td, Ed, Ld, Cd>& dst)
    -> MdArray<Td, Ed, Ld, Cd>& {
  static_assert(dimsCompatible(Ea::static_extent(1), Eb::static_extent(0)),
                "inner dimensions must match: A columns == B rows");
  static_assert(dimsCompatible(Ed::static_extent(0), Ea::static_extent(0)),
                "dst rows mismatch");
  static_assert(dimsCompatible(Ed::static_extent(1), Eb::static_extent(1)),
                "dst columns mismatch");
  ...
  for (std::size_t i = 0; i < m; ++i)
    for (std::size_t j = 0; j < n; ++j) {
      ...
      for (std::size_t p = 0; p < k; ++p)
        sum += static_cast<Acc>(a[i, p]) * static_cast<Acc>(b[p, j]);
...
template <typename Ta, typename Ea, typename La, typename Ca, typename Tb,
          ...
          typename Ld, typename Cd>
  requires(Ea::rank() == 2 && Eb::rank() == 2 && Ed::rank() == 2)
constexpr auto multiplyAdd(const MdArray<Ta, Ea, La, Ca>& a,
                           const MdArray<Tb, Eb, Lb, Cb>& b,
                           MdArray<Td, Ed, Ld, Cd>& dst)
    -> MdArray<Td, Ed, Ld, Cd>& {
  static_assert(dimsCompatible(Ea::static_extent(1), Eb::static_extent(0)),
...
template <typename Ta, typename Ea, typename La, typename Ca, typename Tb,
          typename Eb, typename Lb, typename Cb>
  requires(Ea::rank() == 2 && Eb::rank() == 2)
constexpr auto add(const MdArray<Ta, Ea, La, Ca>& a,
                   const MdArray<Tb, Eb, Lb, Cb>& b) {
  ResultMatrix<std::common_type_t<Ta, Tb>, SumExtents<Ea, Eb>> c(a.extent(0),
                                                                 a.extent(1));
  add(a, b, c);
  return c;
}
...
constexpr auto multiply(const MdArray<Ta, Ea, La, Ca>& a,
                        const MdArray<Tb, Eb, Lb, Cb>& b) {
  ResultMatrix<std::common_type_t<Ta, Tb>, ProductExtents<Ea, Eb>> c(
...
template <typename Ta, typename Ea, typename La, typename Ca, typename Tb,
          typename Eb, typename Lb, typename Cb>
  requires(Ea::rank() == 2 && Eb::rank() == 2)
constexpr auto operator+(const MdArray<Ta, Ea, La, Ca>& a,
                         const MdArray<Tb, Eb, Lb, Cb>& b) {
  return add(a, b);
}
template <typename Ta, typename Ea, typename La, typename Ca, typename Tb,
...
template <typename Ta, typename Ea, typename La, typename Ca, typename Tb,
          typename Eb, typename Lb, typename Cb>
  requires(Ea::rank() == 2 && Eb::rank() == 2)
constexpr auto operator+=(MdArray<Ta, Ea, La, Ca>& a,
                          const MdArray<Tb, Eb, Lb, Cb>& b)
    -> MdArray<Ta, Ea, La, Ca>& {
  return add(a, b, a);
}
...
constexpr auto operator*=(MdArray<Ta, Ea, La, Ca>& a,
                          const MdArray<Tb, Eb, Lb, Cb>& b)
    -> MdArray<Ta, Ea, La, Ca>& {
  static_assert(dimsCompatible(Eb::static_extent(0), Eb::static_extent(1)),
                "b must be square");
  static_assert(dimsCompatible(Ea::static_extent(1), Eb::static_extent(0)),
                "b side != a columns");
  ...
  for (std::size_t i = 0; i < rows; ++i)
    for (std::size_t j = 0; j < cols; ++j)
      a[i, j] = static_cast<Ta>(product[i, j]);
  return a;
}
