...
auto main() -> int {
  "monte_carlo_integration"_test = [] {
    ...
    auto sample = [gen = makeRandom()] mutable {
      constexpr double scale = 1.0 / static_cast<double>(std::numeric_limits<uint64_t>::max());
    ...
    integrator.step(1'000'000);  // Reduced for faster testing
    ...
    // Just check that we got some reasonable numerical values (not NaN or inf)
    // This is primarily testing that the RNG works and integrates properly
