...
// possible completion channels.
template <typename S, typename Env = EmptyEnv>
concept Sender = std::move_constructible<S> &&
                 requires {
                   typename std::remove_cvref_t<S>::template CompletionSignatures<Env>;
                 };
...
// Extract completion signatures from a sender
template <typename S, typename Env = EmptyEnv>
struct GetCompletionSignatures {
  using Type = typename std::remove_cvref_t<S>::template CompletionSignatures<Env>;
};
