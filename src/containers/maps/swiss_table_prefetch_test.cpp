...
auto main() -> int {
  "empty map"_test = [] {
    SwissTablePrefetch<int, int> map;
  ...
  "insert and find"_test = [] {
    SwissTablePrefetch<int, std::string> map;
  ...
  "contains and find"_test = [] {
    SwissTablePrefetch<std::string, int> map;
    map.insert("apple", 1);
    map.insert("banana", 2);
    map.insert("cherry", 3);
  ...
  "high load factor - 87.5% target"_test = [] {
    SwissTablePrefetch<int, int> map{16};
  ...
  "SIMD group matching with prefetching"_test = [] {
    // Prefetching should maintain correctness across group boundaries
    SwissTablePrefetch<int, int> map;
  ...
  "copy constructor"_test = [] {
    SwissTablePrefetch<int, std::string> original;
    original.insert(1, "one");
    original.insert(2, "two");

    SwissTablePrefetch<int, std::string> copy{original};
  ...
  "move constructor"_test = [] {
    SwissTablePrefetch<int, int> original;
    original.insert(1, 100);
    original.insert(2, 200);

    SwissTablePrefetch<int, int> moved{std::move(original)};
  ...
  "copy assignment"_test = [] {
    ...
    SwissTablePrefetch<int, std::string> copy;
  ...
  "move assignment"_test = [] {
    ...
    SwissTablePrefetch<int, int> target;
    target.insert(99, 99);
  ...
  "swap"_test = [] {
    SwissTablePrefetch<int, int> a;
    a.insert(1, 10);
    a.insert(2, 20);

    SwissTablePrefetch<int, int> b;
    b.insert(3, 30);
  ...
  "large scale with prefetching"_test = [] {
    SwissTablePrefetch<int, int> map;
  ...
  "capacity is multiple of 16 (group width)"_test = [] {
    SwissTablePrefetch<int, int> map1{10};
    expectEq(16uz, map1.capacity());

    SwissTablePrefetch<int, int> map2{20};
    expectEq(32uz, map2.capacity());

    SwissTablePrefetch<int, int> map3{33};
    expectEq(64uz, map3.capacity());
  };

  "H2 filtering with prefetching"_test = [] {
    // Prefetching should not interfere with H2 hash filtering
    SwissTablePrefetch<int, int> map;
