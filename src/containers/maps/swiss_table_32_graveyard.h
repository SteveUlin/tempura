...
// ============================================================================
// Swiss Table 32 Graveyard - AVX2 + Strategic Tombstone Placement
// ============================================================================
//
// Combines SwissTable32's AVX2 32-byte group matching with Graveyard Hashing's
// strategic tombstone placement to combat clustering.
//
// ============================================================================
...
class SwissTable32Graveyard {
  static constexpr std::uint8_t kEmpty = swiss32gr::kEmpty;
  static constexpr std::uint8_t kDeleted = swiss32gr::kDeleted;
  using Group = swiss32gr::Group;
  using BitMask = swiss32gr::BitMask;
  using ProbeSeq = swiss32gr::ProbeSeq;

  static constexpr auto h2(std::size_t hash) -> std::uint8_t {
    return swiss32gr::h2(hash);
  }
 ...
 public:
  ...
  constexpr SwissTable32Graveyard() : SwissTable32Graveyard{kMinCapacity} {}

  constexpr explicit SwissTable32Graveyard(size_type initial_capacity)
  ...
  constexpr SwissTable32Graveyard(const SwissTable32Graveyard& other)
  ...
  constexpr SwissTable32Graveyard(SwissTable32Graveyard&& other) noexcept
      : capacity_{other.capacity_},
        size_{other.size_},
        deleted_{other.deleted_},
  ...
  constexpr auto operator=(const SwissTable32Graveyard& other) -> SwissTable32Graveyard& {
    if (this != &other) {
      SwissTable32Graveyard tmp{other};
      swap(tmp);
    }
    return *this;
  }

  constexpr auto operator=(SwissTable32Graveyard&& other) noexcept -> SwissTable32Graveyard& {
    if (this != &other) {
      SwissTable32Graveyard tmp{std::move(other)};
      swap(tmp);
    }
    return *this;
  }

  constexpr ~SwissTable32Graveyard() {
    if (slots_) {
      for (size_type i = 0; i < capacity_; ++i) {
        if (isOccupied(ctrl_[i])) {
          slots_[i].destroy();
  ...
  constexpr void swap(SwissTable32Graveyard& other) noexcept {
