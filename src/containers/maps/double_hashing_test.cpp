...
auto main() -> int {
  ...
  "insert and find"_test = [] {
    DoubleHashingMap<int, std::string> map;
  ...
  "contains and find"_test = [] {
    DoubleHashingMap<std::string, int> map;
    map.insert("apple", 1);
    map.insert("banana", 2);
    map.insert("cherry", 3);
  ...
  "collision handling"_test = [] {
    // Use a trivial hash that causes collisions for both h1 and h2
    struct BadHash1 {
      auto operator()(int key) const -> std::size_t {
        return static_cast<std::size_t>(key % 4);  // Only 4 buckets worth
      }
    };

    struct BadHash2 {
      auto operator()(int /*key*/) const -> std::size_t {
        return 1;  // Constant step size degrades to linear probing
      }
    };

    DoubleHashingMap<int, int, BadHash1, BadHash2> map;
  ...
  "resize triggers"_test = [] {
    DoubleHashingMap<int, int> map{11};  // Start with capacity 11 (first prime)

    // Insert enough to trigger resize (load factor > 0.7)
    // 11 * 0.7 = 7.7, so inserting 8+ should trigger resize
    for (int i = 0; i < 20; ++i) {
      map.insert(i, i * 10);
    }
  ...
  "copy constructor"_test = [] {
    DoubleHashingMap<int, std::string> original;
    original.insert(1, "one");
    original.insert(2, "two");

    DoubleHashingMap<int, std::string> copy{original};
  ...
  "move constructor"_test = [] {
    DoubleHashingMap<int, int> original;
    original.insert(1, 100);
    original.insert(2, 200);

    DoubleHashingMap<int, int> moved{std::move(original)};
  ...
  "copy assignment"_test = [] {
    ...
    DoubleHashingMap<int, std::string> copy;
  ...
  "move assignment"_test = [] {
    ...
    DoubleHashingMap<int, int> target;
    target.insert(99, 99);
  ...
  "swap"_test = [] {
    DoubleHashingMap<int, int> a;
    a.insert(1, 10);
    a.insert(2, 20);

    DoubleHashingMap<int, int> b;
    b.insert(3, 30);
  ...
  "custom hash functions"_test = [] {
    // Custom primary hash that uses higher bits
    struct CustomHash1 {
      auto operator()(int key) const -> std::size_t {
    ...
    // Custom secondary hash for step size
    struct CustomHash2 {
      auto operator()(int key) const -> std::size_t {
        // Different prime multiplier, ensure non-zero
        std::size_t h = static_cast<std::size_t>(key) * 1664525ULL;
        return (h % 5) + 1;  // Returns value in [1, 5]
      }
    };

    DoubleHashingMap<int, std::string, CustomHash1, CustomHash2> map;
    ...
    expectEq(5uz, map.size());
    expectEq(std::string{"one"}, *map.find(1));
    expectEq(std::string{"two"}, *map.find(2));
    expectEq(std::string{"three"}, *map.find(3));
    expectEq(std::string{"hundred"}, *map.find(100));
    expectEq(std::string{"thousand"}, *map.find(1000));

    // Test erase with custom hashes
    map.erase(2);
    expectFalse(map.contains(2));
    expectEq(4uz, map.size());
