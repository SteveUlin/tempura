...
auto main() -> int {
  ...
  "neighborhood bitmap lookup"_test = [] {
    // Hopscotch uses bitmap to track which slots belong to each bucket
    // This test ensures colliding keys are found correctly
    struct BadHash {
    ...
    HopscotchMap<int, int, BadHash> map;
    ...
    expectEq(16, *map.find(16));
    expectEq(24, *map.find(24));
  };

  "hopping for distant slots"_test = [] {
    // When empty slot is beyond neighborhood, hopscotch moves elements
    HopscotchMap<int, int> map;

    // Insert many elements to trigger hopping
    for (int i = 0; i < 100; ++i) {
      map.insert(i, i * 10);
    }
  ...
  "copy constructor"_test = [] {
    HopscotchMap<int, std::string> original;
    original.insert(1, "one");
    original.insert(2, "two");

    HopscotchMap<int, std::string> copy{original};
  ...
  "move constructor"_test = [] {
    HopscotchMap<int, int> original;
    original.insert(1, 100);
    original.insert(2, 200);

    HopscotchMap<int, int> moved{std::move(original)};
  ...
  "copy assignment"_test = [] {
    ...
    HopscotchMap<int, std::string> copy;
  ...
  "swap"_test = [] {
    HopscotchMap<int, int> a;
    a.insert(1, 10);
    a.insert(2, 20);

    HopscotchMap<int, int> b;
    b.insert(3, 30);
