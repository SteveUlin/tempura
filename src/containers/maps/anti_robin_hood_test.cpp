...
auto main() -> int {
  ...
  "insert and find"_test = [] {
    AntiRobinHoodMap<int, std::string> map;
  ...
  "contains and find"_test = [] {
    AntiRobinHoodMap<std::string, int> map;
    map.insert("apple", 1);
    map.insert("banana", 2);
    map.insert("cherry", 3);
  ...
  "collision handling"_test = [] {
    ...
    AntiRobinHoodMap<int, int, BadHash> map;
  ...
  "resize triggers"_test = [] {
    AntiRobinHoodMap<int, int> map{8};
  ...
  "copy constructor"_test = [] {
    AntiRobinHoodMap<int, std::string> original;
    original.insert(1, "one");
    original.insert(2, "two");

    AntiRobinHoodMap<int, std::string> copy{original};
  ...
  "move constructor"_test = [] {
    AntiRobinHoodMap<int, int> original;
    original.insert(1, 100);
    original.insert(2, 200);

    AntiRobinHoodMap<int, int> moved{std::move(original)};
  ...
  "copy assignment"_test = [] {
    ...
    AntiRobinHoodMap<int, std::string> copy;
  ...
  "move assignment"_test = [] {
    ...
    AntiRobinHoodMap<int, int> target;
    target.insert(99, 99);
  ...
  "swap"_test = [] {
    AntiRobinHoodMap<int, int> a;
    a.insert(1, 10);
    a.insert(2, 20);

    AntiRobinHoodMap<int, int> b;
    b.insert(3, 30);
  ...
  "displacement statistics - pathological"_test = [] {
    // Anti-Robin Hood should have HIGH max displacement
    struct BadHash {
      auto operator()(int key) const -> std::size_t {
    ...
    AntiRobinHoodMap<int, int, BadHash> map{64};

    for (int i = 0; i < 40; ++i) {
      map.insert(i, i);
    }

    // Anti-Robin Hood causes displacement to compound
    // We expect some elements to be pushed very far from home
    // This is the pathological case we're demonstrating
    expectGT(map.maxDisplacement(), 5uz);
  };
