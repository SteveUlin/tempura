#include "containers/maps/coalesced.h"
...
auto main() -> int {
  ...
  "insert and find"_test = [] {
    CoalescedMap<int, std::string> map;
  ...
  "contains and find"_test = [] {
    CoalescedMap<std::string, int> map;
    map.insert("apple", 1);
    map.insert("banana", 2);
    map.insert("cherry", 3);
  ...
  "collision handling - chains in table"_test = [] {
    // Coalesced hashing stores collision chains within the table
    CoalescedMap<int, int> map{16};

    // Insert keys that will collide (same hash % address_limit)
    // With address_limit = 13 (86% of 16), keys 0, 13, 26 hash to same slot
    map.insert(0, 100);
    map.insert(13, 130);
    map.insert(26, 260);

    expectEq(3uz, map.size());
    expectEq(100, *map.find(0));
    expectEq(130, *map.find(13));
    expectEq(260, *map.find(26));

    // All three keys should be findable despite collisions
    expectTrue(map.contains(0));
    expectTrue(map.contains(13));
    expectTrue(map.contains(26));
  };

  "tombstones preserve chains"_test = [] {
    CoalescedMap<int, int> map{16};
    ...
    // Delete middle element - tombstone should keep chain intact
    expectTrue(map.erase(13));

    expectTrue(map.contains(0));
    expectFalse(map.contains(13));
    expectTrue(map.contains(26));
  ...
  "many collisions"_test = [] {
    CoalescedMap<int, int> map;

    // Insert many elements to test chain behavior
    for (int i = 0; i < 100; ++i) {
      map.insert(i, i * 10);
    }
  ...
  "resize maintains chains"_test = [] {
    CoalescedMap<int, int> map{8};
  ...
  "copy constructor"_test = [] {
    CoalescedMap<int, std::string> original;
    original.insert(1, "one");
    original.insert(2, "two");

    CoalescedMap<int, std::string> copy{original};
  ...
  "move constructor"_test = [] {
    CoalescedMap<int, int> original;
    original.insert(1, 100);
    original.insert(2, 200);

    CoalescedMap<int, int> moved{std::move(original)};
  ...
  "copy assignment"_test = [] {
    ...
    CoalescedMap<int, std::string> copy;
  ...
  "swap"_test = [] {
    CoalescedMap<int, int> a;
    a.insert(1, 10);
    a.insert(2, 20);

    CoalescedMap<int, int> b;
    b.insert(3, 30);
  ...
  "chain length analysis"_test = [] {
    CoalescedMap<int, int> map{32};
    ...
    // With coalesced hashing, chains should be relatively short
    auto max_chain = map.maxChainLength();
    auto avg_chain = map.avgChainLength();

    // Sanity checks - chains exist but aren't pathological
    expectGE(max_chain, 1uz);
    expectGE(avg_chain, 1.0);
    expectLE(avg_chain, 10.0);  // Should be much better than this
  };
