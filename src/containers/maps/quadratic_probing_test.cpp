...
auto main() -> int {
  "empty map"_test = [] {
    QuadraticProbingMap<int, int> map;
  ...
  "insert and find"_test = [] {
    QuadraticProbingMap<int, std::string> map;
  ...
  "contains and find"_test = [] {
    QuadraticProbingMap<std::string, int> map;
    map.insert("apple", 1);
    map.insert("banana", 2);
    map.insert("cherry", 3);
  ...
  "collision handling"_test = [] {
    ...
    QuadraticProbingMap<int, int, BadHash> map;
  ...
  "tombstone handling"_test = [] {
    ...
    // Insert A, B, C - all hash to 0
    // With triangular probing: slot 0, 0+1=1, 0+3=3
    map.insert(10, 100);
    map.insert(20, 200);
    map.insert(30, 300);
  ...
  "quadratic probe sequence visits all slots"_test = [] {
    // Verify that triangular probing visits all slots
    // For capacity=8, starting at 0: 0, 1, 3, 6, 2, 7, 5, 4 (all distinct mod 8)
    struct BadHash {
      auto operator()(int key) const -> std::size_t {
        return 0;  // Everything hashes to slot 0
      }
    ...
    QuadraticProbingMap<int, int, BadHash> map{8};

    // Insert 5 elements (load factor will be 5/8 = 0.625, under 0.7 threshold)
    for (int i = 0; i < 5; ++i) {
      map.insert(i, i * 10);
    }
  ...
  "resize triggers"_test = [] {
    QuadraticProbingMap<int, int> map{8};  // Start with capacity 8
  ...
  "capacity is power of 2"_test = [] {
    // Verify capacity is always power of 2 (required for triangular probing)
    QuadraticProbingMap<int, int> map1{1};
    expectEq(8uz, map1.capacity());  // Rounds up to kMinCapacity

    QuadraticProbingMap<int, int> map2{9};
    expectEq(16uz, map2.capacity());  // 9 rounds up to 16

    QuadraticProbingMap<int, int> map3{17};
    expectEq(32uz, map3.capacity());  // 17 rounds up to 32
  };
  ...
  "copy constructor"_test = [] {
    QuadraticProbingMap<int, std::string> original;
    original.insert(1, "one");
    original.insert(2, "two");

    QuadraticProbingMap<int, std::string> copy{original};
  ...
  "move constructor"_test = [] {
    QuadraticProbingMap<int, int> original;
    original.insert(1, 100);
    original.insert(2, 200);

    QuadraticProbingMap<int, int> moved{std::move(original)};
  ...
  "copy assignment"_test = [] {
    ...
    QuadraticProbingMap<int, std::string> copy;
  ...
  "move assignment"_test = [] {
    ...
    QuadraticProbingMap<int, int> target;
    target.insert(99, 99);
  ...
  "swap"_test = [] {
    QuadraticProbingMap<int, int> a;
    a.insert(1, 10);
    a.insert(2, 20);

    QuadraticProbingMap<int, int> b;
    b.insert(3, 30);
