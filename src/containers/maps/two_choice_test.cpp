...
auto main() -> int {
  ...
  "insert and find"_test = [] {
    TwoChoiceMap<int, std::string> map;
  ...
  "contains and find"_test = [] {
    TwoChoiceMap<std::string, int> map;
    map.insert("apple", 1);
    map.insert("banana", 2);
    map.insert("cherry", 3);
  ...
  "two-choice load balancing"_test = [] {
    // Power of two choices should keep max bucket size low
    TwoChoiceMap<int, int> map{16};
    ...
    // With 100 elements in 16 buckets, average load is ~6.25
    // Two-choice should keep max load much lower than random placement
    // Random: max ~ O(log n / log log n) ~ 6-8
    // Two-choice: max ~ O(log log n) ~ 3-4
    expectLE(map.maxBucketSize(), 15uz);
  };

  "resize triggers"_test = [] {
    TwoChoiceMap<int, int> map{8};
  ...
  "copy constructor"_test = [] {
    TwoChoiceMap<int, std::string> original;
    original.insert(1, "one");
    original.insert(2, "two");

    TwoChoiceMap<int, std::string> copy{original};
  ...
  "move constructor"_test = [] {
    TwoChoiceMap<int, int> original;
    original.insert(1, 100);
    original.insert(2, 200);

    TwoChoiceMap<int, int> moved{std::move(original)};
  ...
  "copy assignment"_test = [] {
    ...
    TwoChoiceMap<int, std::string> copy;
  ...
  "swap"_test = [] {
    TwoChoiceMap<int, int> a;
    a.insert(1, 10);
    a.insert(2, 20);

    TwoChoiceMap<int, int> b;
    b.insert(3, 30);
