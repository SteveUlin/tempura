...
class SwissTable {
 public:
  ...
  constexpr auto erase(const Key& key) -> bool {
    ...
    // For Swiss Tables with group-based quadratic probing, we can't simply
    // check if the next linear slot is empty to determine if this is the end
    // of a probe chain. The probe sequence visits groups of 16, not individual
    // slots, so we must use a tombstone to preserve findability of all keys
    // that may have probed past this position.
    ctrl_[idx] = kDeleted;
    --size_;
    ++deleted_;
