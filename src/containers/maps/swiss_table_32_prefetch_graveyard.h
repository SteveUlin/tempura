...
// ============================================================================
// Swiss Table 32 Prefetch Graveyard - AVX2 + Prefetch + Tombstone Strategy
// ============================================================================
//
// The ultimate combination: AVX2 32-byte groups, software prefetching for
// memory latency hiding, and strategic tombstone placement for clustering.
//
// ============================================================================
...
class SwissTable32PrefetchGraveyard {
  static constexpr std::uint8_t kEmpty = swiss32pfgr::kEmpty;
  static constexpr std::uint8_t kDeleted = swiss32pfgr::kDeleted;
  using Group = swiss32pfgr::Group;
  using BitMask = swiss32pfgr::BitMask;
  using ProbeSeq = swiss32pfgr::ProbeSeq;

  static constexpr auto h2(std::size_t hash) -> std::uint8_t {
    return swiss32pfgr::h2(hash);
  }
 ...
 public:
  ...
  constexpr SwissTable32PrefetchGraveyard() : SwissTable32PrefetchGraveyard{kMinCapacity} {}

  constexpr explicit SwissTable32PrefetchGraveyard(size_type initial_capacity)
  ...
  constexpr SwissTable32PrefetchGraveyard(const SwissTable32PrefetchGraveyard& other)
  ...
  constexpr SwissTable32PrefetchGraveyard(SwissTable32PrefetchGraveyard&& other) noexcept
      : capacity_{other.capacity_},
        size_{other.size_},
        deleted_{other.deleted_},
  ...
  constexpr auto operator=(const SwissTable32PrefetchGraveyard& other) -> SwissTable32PrefetchGraveyard& {
    if (this != &other) {
      SwissTable32PrefetchGraveyard tmp{other};
      swap(tmp);
    }
    return *this;
  }

  constexpr auto operator=(SwissTable32PrefetchGraveyard&& other) noexcept -> SwissTable32PrefetchGraveyard& {
    if (this != &other) {
      SwissTable32PrefetchGraveyard tmp{std::move(other)};
      swap(tmp);
    }
    return *this;
  }

  constexpr ~SwissTable32PrefetchGraveyard() {
    if (slots_) {
      for (size_type i = 0; i < capacity_; ++i) {
        if (isOccupied(ctrl_[i])) {
          slots_[i].destroy();
  ...
  constexpr void swap(SwissTable32PrefetchGraveyard& other) noexcept {
 ...
 private:
  ...
  // insertSlot with prefetching + graveyard
