...
auto main() -> int {
  ...
  "contains and find"_test = [] {
    SwissTable64<std::string, int> map;
    map.insert("apple", 1);
    map.insert("banana", 2);
    map.insert("cherry", 3);
  ...
  "high load factor - 87.5% target"_test = [] {
    SwissTable64<int, int> map{64};

    // Fill to ~87% (56 elements in 64 slots)
    for (int i = 0; i < 56; ++i) {
      map.insert(i, i * 10);
    }

    expectEq(56uz, map.size());
    expectGE(map.loadFactor(), 0.8);
    expectLE(map.loadFactor(), 0.9);
  ...
  "64-byte AVX-512 group matching"_test = [] {
    SwissTable64<int, int> map;

    // Insert elements that would span multiple slots within a 64-byte group
    for (int i = 0; i < 100; ++i) {
      map.insert(i, i);
    }
  ...
  "copy constructor"_test = [] {
    SwissTable64<int, std::string> original;
    original.insert(1, "one");
    original.insert(2, "two");

    SwissTable64<int, std::string> copy{original};
  ...
  "move constructor"_test = [] {
    SwissTable64<int, int> original;
    original.insert(1, 100);
    original.insert(2, 200);

    SwissTable64<int, int> moved{std::move(original)};
  ...
  "copy assignment"_test = [] {
    ...
    SwissTable64<int, std::string> copy;
  ...
  "swap"_test = [] {
    SwissTable64<int, int> a;
    a.insert(1, 10);
    a.insert(2, 20);

    SwissTable64<int, int> b;
    b.insert(3, 30);
  ...
  "capacity is multiple of 64"_test = [] {
    // Capacity must be power of 2 (for bitwise AND masking in ProbeSeq64)
    // and at least Group64::kWidth (64) for AVX-512 alignment
    SwissTable64<int, int> map1{1};
    expectEq(64uz, map1.capacity());

    SwissTable64<int, int> map2{65};
    expectEq(128uz, map2.capacity());  // Next power of 2 after 65

    SwissTable64<int, int> map3{100};
    expectEq(128uz, map3.capacity());  // Next power of 2 after 100
  };
