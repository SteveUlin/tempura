...
auto main() -> int {
  ...
  "O(1) lookup - cuckoo checks exactly 2 slots"_test = [] {
    // Cuckoo's key property: lookup is O(1) worst case
    // Each key can only be in one of two slots
    CuckooMap<int, int> map;
    ...
    // Non-existent keys are quickly determined
    for (int i = 100; i < 200; ++i) {
      expectFalse(map.contains(i));
    }
  };

  "displacement handling"_test = [] {
    // Cuckoo kicks out occupants, creating displacement chains
    CuckooMap<int, int> map;

    // Insert enough elements to trigger displacements
    for (int i = 0; i < 50; ++i) {
      map.insert(i, i);
    }
  ...
  "resize triggers on cycle detection"_test = [] {
    CuckooMap<int, int> map{8};

    // With a small table, cycles are likely - should trigger resize
    for (int i = 0; i < 30; ++i) {
      map.insert(i, i * 10);
    }
  ...
  "copy constructor"_test = [] {
    CuckooMap<int, std::string> original;
    original.insert(1, "one");
    original.insert(2, "two");

    CuckooMap<int, std::string> copy{original};
  ...
  "move constructor"_test = [] {
    CuckooMap<int, int> original;
    original.insert(1, 100);
    original.insert(2, 200);

    CuckooMap<int, int> moved{std::move(original)};
