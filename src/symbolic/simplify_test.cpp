...
auto main() -> int {
  "Plus Reduce"_test = [] {
    ...
    // std::cout << toString(
    //   reduceAddition(a + b + a), Substitution{a = "a", b = "b"}) << std::endl;
    ...
    std::cout << typeName(reduceMultiplication(a * a)) << std::endl;

    // std::cout << toString(simplify(a + Constant<4>{} * b * b), Substitution{a = "a", b = "b"}) << std::endl;
  };
