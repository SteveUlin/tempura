...
auto main() -> int {
  ...
  "permutedRows materializes into contiguous storage"_test = [] {
    ...
    Dense<double, Dyn, Dyn> r = permutedRows(m, p);
    expectEq(r[0, 0], 30.0);
    expectEq(r[1, 0], 10.0);
    expectEq(r[2, 0], 20.0);
  };
  ...
  "permuted view composes with multiply (zero-copy row reorder)"_test = [] {
    ...
    const Dense<double, Dyn, Dyn> bperm = permutedRows(b, p);  // materialized reference
  ...
  "eager permuteRows agrees with the lazy permutedRows view"_test = [] {
    Dense<double, Dyn, Dyn> m(dims(4, 2));
    for (std::size_t i = 0; i < 4; ++i) {
      m[i, 0] = static_cast<double>(i);
      m[i, 1] = static_cast<double>(100 + i);
    }
    const Permutation<4> p{2, 0, 3, 1};
    Dense<double, Dyn, Dyn> eager = m;        // copy, then gather in place
    p.permuteRows(eager);
