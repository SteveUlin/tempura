...
int main() {
  std::cout << "=== Repeated Pattern Variables Test ===" << std::endl
            << std::endl;
  ...
  // Test 3: pow(x_, a_) * pow(x_, b_) should match pow(a, 2) * pow(a, 3) but
  // not pow(a, 2) * pow(b, 3)
  std::cout << std::endl
            << "Testing pow(x_, a_) * pow(x_, b_) pattern..." << std::endl;
  {
  ...
  std::cout << std::endl << "Testing complex nested pattern..." << std::endl;
  {
    ...
    std::cout << "  ✓ (a + b) * (a + c) → (a + b) * (a + c) (no match)"
              << std::endl;
  }

  std::cout << std::endl
            << "=== All repeated variable tests passed! ===" << std::endl;
  return 0;
}
