...
auto main() -> int {
  ...
  // ===========================================================================
  // TransformedPosterior tests
  // ===========================================================================

  "TransformedPosterior with positive transform"_test = [] {
    // Model: sigma ~ HalfNormal(2)
    auto sigma = halfNormal(2.0);
    auto lp = logProb(sigma);

    // Create transformed posterior (sigma must be positive)
    auto posterior = makeTransformedPosterior(lp, positive(sigma)).build();

    // Test at z=0 (sigma=1)
    std::array<double, 1> z = {0.0};
    double sigma_val = posterior.transformOne<0>(z[0]);
    expectNear(1.0, sigma_val, 1e-10);

    // Log-prob should include Jacobian
    double log_p = posterior.logProb(z);
    // Should be finite
    expectTrue(std::isfinite(log_p));
  };

  "TransformedPosterior with multiple transforms"_test = [] {
    // Model: mu ~ Normal(0, 5), sigma ~ HalfNormal(2)
    auto mu = normal(lit(0.0), lit(5.0));
    auto sigma = halfNormal(2.0);
    auto lp = logProb(mu, sigma);

    // mu is unconstrained, sigma is positive
    ...
    std::array<double, 2> z = {1.0, 0.0};  // mu=1, sigma=exp(0)=1
  ...
  "TransformedPosterior inverse transform"_test = [] {
    auto sigma = halfNormal(2.0);
    auto lp = logProb(sigma);
    auto posterior = makeTransformedPosterior(lp, positive(sigma)).build();

    // Start in constrained space
    std::array<double, 1> x = {2.5};
    auto z = posterior.inverse(x);
    expectNear(std::log(2.5), z[0], 1e-10);

    // Round-trip
    auto x_back = posterior.transform(z);
    expectNear(x[0], x_back[0], 1e-10);
  };

  "TransformedPosterior gradient includes Jacobian"_test = [] {
    // Simple test: sigma ~ HalfNormal(1)
    // log p(sigma) = -sigma^2/2 (unnormalized)
    // In z space: sigma = exp(z), so:
    //   log p(z) = -exp(2z)/2 + z  (Jacobian)
    // d/dz = -exp(2z) + 1
    ...
    // At z=0: sigma=1, d/dz = -1 + 1 = 0
    // But this depends on the exact form of unnormalized log-prob
    // Just check it's finite and reasonable
    expectTrue(std::isfinite(grad[0]));
  };

  "UnitInterval in TransformedPosterior"_test = [] {
    // Model: p ~ Beta(2, 3) (probability parameter)
    auto p = beta(lit(2.0), lit(3.0));
    auto lp = logProb(p);

    auto posterior = makeTransformedPosterior(lp, unitInterval(p)).build();

    // At z=0: p = sigmoid(0) = 0.5
    std::array<double, 1> z = {0.0};
    double p_val = posterior.transformOne<0>(z[0]);
    expectNear(0.5, p_val, 1e-10);
  ...
  // ===========================================================================
  // Gradient tests for bounded transforms
  // ===========================================================================

  "LowerBounded gradient is analytic"_test = [] {
    // Model: x ~ Exponential(1) has support (0, inf)
    // Use lowerBounded with lower=0 which is equivalent to positive
    auto x = exponential(1.0);
    auto lp = logProb(x);
    auto posterior = makeTransformedPosterior(lp, lowerBounded(x, 0.0)).build();
    ...
    double lp_plus = posterior.logProb({z[0] + eps});
    double lp_minus = posterior.logProb({z[0] - eps});
    double fd_grad = (lp_plus - lp_minus) / (2 * eps);
    expectNear(fd_grad, grad[0], 1e-4);
  };

  "UpperBounded gradient is analytic"_test = [] {
    // Use Normal distribution with upperBounded transform
    // Model: x ~ Normal(5, 1) with upper bound at 10
    auto x = normal(lit(5.0), lit(1.0));
    auto lp = logProb(x);
    auto posterior = makeTransformedPosterior(lp, upperBounded(x, 10.0)).build();

    std::array<double, 1> z = {1.0};  // x = 10 - exp(1) ≈ 7.28
    auto grad = posterior.gradient(z);
    expectTrue(std::isfinite(grad[0]));
  ...
  "Interval gradient is analytic"_test = [] {
    // Use Beta distribution with interval transform (though beta naturally uses unitInterval)
    // Model: p ~ Beta(2, 3) constrained to (0, 1)
    auto p = beta(lit(2.0), lit(3.0));
    auto lp = logProb(p);
    auto posterior = makeTransformedPosterior(lp, interval(p, 0.0, 1.0)).build();

    std::array<double, 1> z = {0.0};  // p = 0.5
    auto grad = posterior.gradient(z);
    expectTrue(std::isfinite(grad[0]));
